...
operation = "CV"
test = "Line OVP"
condition = "CC point = 1.8 A, 20 V (Rls = 7.1 MOhms)"
...
def collect_data(start_voltage, end_voltage, load):
    ...
    input_step = f"{start_voltage}-{end_voltage}"
...
def export_to_excel(df, start_voltage, end_voltage, load):
...
def line_transient_test(df, start_voltage, end_voltage, load):
    ...
    screenshot(filename, path)
    export_to_excel(df, start_voltage, end_voltage, load)
...
def line_ovp(df, start_voltage, end_voltage, load):

    scope.time_position(10)
    scope.time_scale(10)
    scope.run_single()
    scope.edge_trigger(vin_channel, 265, 'POS')


    if load == 0:
        eload.channel[eload_channel].turn_off()
        filename = f"{test}, {start_voltage}-{end_voltage}Vac, NL.png"
    else:
        if load_type == 'cc':
        ...
        if load_type == 'cr':
            ...
            eload.channel[eload_channel].turn_on()
            percent = int(cr_load*100/load)
            filename = f"{test}, {start_voltage}-{end_voltage}Vac, {load:.2f} Ohms ({percent} CR load).png"

    soak(15)
    browning(start_voltage, end_voltage, 0.5, 60)
    ac.turn_off()
    soak(25)
...
def operation():
    ...
    if load_type == 'cc':
        for iout in iout_list:
            line_ovp(df, start_voltage=270, end_voltage=296, load=iout)
            line_ovp(df, start_voltage=286, end_voltage=260, load=iout)
            line_ovp(df, start_voltage=270, end_voltage=300, load=iout)
            line_ovp(df, start_voltage=300, end_voltage=260, load=iout)


    if load_type == 'cr':
        for cr in cr_list:
            line_ovp(df, start_voltage=270, end_voltage=296, load=cr)
            line_ovp(df, start_voltage=286, end_voltage=260, load=cr)
            line_ovp(df, start_voltage=270, end_voltage=300, load=cr)
            line_ovp(df, start_voltage=300, end_voltage=260, load=cr)
