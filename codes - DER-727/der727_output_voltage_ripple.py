...
def scope_settings():
    ...
    scope.channel_settings(state='ON', channel=2, scale=0.02, position=0, label="Output Voltage Ripple",
                            color='LIGHT_BLUE', rel_x_position=40, bandwidth=500, coupling='AC', offset=0)
    
    
    scope.channel_settings(state='OFF', channel=3, scale=0.02, position=-5, label="Output Current",
                            color='YELLOW', rel_x_position=40, bandwidth=20, coupling='DCLimit', offset=1.5)
...
def collect_data(led, v_input):
    ...
    vout_ripple = 100*vout_pkpk/led
    output_list = [load, vac, vout_pkpk, vout_ripple]
    
    return output_list

def operation():
    df_header_list = ['LED', 'Vin', 'Vpk-pk (V)', '%Peak to Peak (%)']
    ...
    for led in led_list:
        ...
        for vin in vin_list:
            ...
            find_trigger(channel=vout_channel, trigger_delta=0.001)
            ...
            discharge_output()
            filename = f"Vout Ripple - {led}V, {vin}Vac.png"
