...
####################################################################################################################################################################
test = "LYTSwitch-6 Drain Voltage and Current Normal"
...
def operation():
    ...
    for led in led_list:
        ...
        for vin in vin_list:
            ...
            filename = f"Vds Ids Normal {time_scale}s_per_div - {led}V, {vin}Vac.png"
            screenshot(filename, path)


            scope.time_scale(zoomed_time_scale)
            sleep(2)
            filename = f"Vds Ids Normal {zoomed_time_scale}s_per_div - {led}V, {vin}Vac.png"
            screenshot(filename, path)
            scope.time_scale(time_scale)
