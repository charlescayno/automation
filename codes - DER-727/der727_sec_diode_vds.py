...
def scope_settings():
    ...
    scope.channel_settings(state='ON', channel=2, scale=40, position=-4, label="Drain Voltage",
                            color='LIGHT_BLUE', rel_x_position=40, bandwidth=500, coupling='DCLimit', offset=0)
    
    
    scope.channel_settings(state='OFF', channel=3, scale=1, position=-3, label="Drain Current",
...
def operation():
    ...
    for led in led_list:
        ...
        for vin in vin_list:
            ...
            find_trigger(channel=trigger_channel, trigger_delta=1)
