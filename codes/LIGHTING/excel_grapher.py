...
def read_excel_data(file_path):
    df = pd.read_excel(file_path, engine='openpyxl', skiprows=1)  # Skip first row (headers)
    df = df.iloc[:, 1:]  # Start from column B
    last_row = df.shape[0]
    last_col = df.shape[1]
    df = df.iloc[:last_row, :last_col]  # Ends with the shape of DataFrame
    print(df)
    return df
