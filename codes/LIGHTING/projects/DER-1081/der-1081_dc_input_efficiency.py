...
test_name = f"DCInEff-wCV1Load-iter15"
# component = "Primary VDS"
component = "SRFET"
unit = f"3"
...
def main():
    ...
    if component == "SRFET":
        ...
        # scope.write("MMEM:RCL 'C:/Users/Public/Documents/Rohde-Schwarz/RTx/SaveSets/2025/DER-1081 SRFET Vds Vgs Steadystate.dfl'")
        scope.write("MMEM:RCL 'C:/Users/Public/Documents/Rohde-Schwarz/RTx/SaveSets/2025/DER-1081 SRFET Vds SS.dfl'")
    if component == "Primary VDS":
    ...
    # adds scope labels to header_list list variable
    ...
    for vin in vin_list_dc:
        ...
        for iout2 in iout2_list:
            ...
            EQUIPMENT_FUNCTIONS().DC_TURN_ON(vin)
            soak(soak_time)
            # EQUIPMENT_FUNCTIONS().FIND_TRIGGER(channel_to_trigger, channel_trigger_delta)
            EQUIPMENT_FUNCTIONS().SCOPE().RUN_SINGLE()
            soak(2)
            filename = f"{test_name} {vin}VDC {vout_nom_1}V{iout_nom_1}A {vout_nom_2}V{iout2}A"
            EQUIPMENT_FUNCTIONS().SCOPE().STOP()
            EQUIPMENT_FUNCTIONS().SCOPE().SCOPE_SCREENSHOT(filename, waveforms_folder)

            # output_list = EQUIPMENT_FUNCTIONS().COLLECT_DATA_1CV_1CC_PARAMETRICS(vin*1.41421356237, vout_nom_1, vout_nom_2, iout_nom_1, iout_nom_2, scope_channel_list)
