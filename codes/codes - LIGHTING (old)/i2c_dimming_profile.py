...
i2c_type = sys.argv[4] # linear, logartihmic

cp = int(sys.argv[5]) # 0, 1, 2, 3

if i2c_type == 'logarithmic' and cp == 0: step = 1
else: step = 3
    ...
    if test in test_list:
        ...
        for LED in led_list:
            ...
            for vin in vin_list:    
                ...
                print(cp)
                i2c_dimming(LED = LED, vin = vin, start = 0, end = 254, step = step, cp = cp, i2c_type=i2c_type, soak = 10, delay_per_line = 1, delay_per_load = 1)
    ...
    if test in test_list:
        ...
        for LED in led_list:
            ...
            for vin in vin_list:    
                ...
                ate_gui.click(vin_dict[f"{LED}V {vin}Vac"])
                print(cp)
                i2c_dimming(LED = LED, vin = vin, start = 254, end = 0, step = step, cp = cp, i2c_type=i2c_type, soak = 10, delay_per_line = 1, delay_per_load = 1)
