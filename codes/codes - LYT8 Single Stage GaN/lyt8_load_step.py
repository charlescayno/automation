...
cr_load = pout/(iout_nom*iout_nom)
...
# def get_cursor(cursor, attribute):
#     return float(scope.get_cursor(cursor)[attribute])

# def move_cursor(cursor, attribute):
#     scope.cursor(channel=3, cursor_set=cursor, X1=1, X2=1, Y1=0, Y2=0, type='VERT')
...
def loadstep_cr(df, input, cr_load, start_percent, end_percent):
    ...
    print(df)
    export_to_excel(df, waveforms_folder, output_list, excel_name, sheet_name=f"summary_{test}", anchor="B2")
    print()
...
def operation():
    ...
    scope.cursor(channel=2, cursor_set=1, X1=1, X2=1, Y1=50*0.92, Y2=50*1.08, type='HOR')
    scope.cursor(channel=4, cursor_set=2, X1=1, X2=1, Y1=36*0.95, Y2=36*1.05, type='HOR')

    asd = 0


    perm = permutations(percent_list, 2)
    percent_list_perm_len = len(list(perm))

    estimated_time = ( percent_list_perm_len*(180) )/60
