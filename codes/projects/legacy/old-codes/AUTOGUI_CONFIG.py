...
class AutoguiCalibrate():
    ...
    def test_selection(self):
        ...
        self.select_test_line_load_regulation = self.get_coordinates('SELECT_LINE_LOAD_REGULATION')
        self.CVCC_LINE_LOAD_REGULATION()

    # 03OCT2021
    def CVCC_LINE_LOAD_REGULATION(self):
        self.select_regulation_test_exit_button = self.get_coordinates('SELECT_REGULATION_TEST_EXIT_BUTTON')
        self.load_enable_button = self.get_coordinates('LOAD_ENABLE_BUTTON')
        self.load_disable = self.get_coordinates('LOAD_DISABLE')
    ...
    def flicker_digital_control(self):
        self.test_selection()
        self.settings = self.get_coordinates("SETTINGS")
        self.initialize_com_port = self.get_coordinates('INITIALIZE_COM_PORT')
        self.multiprotocol_control_exit_button = self.get_coordinates('MULTIPROTOCOL_CONTROL_EXIT_BUTTON')
    ...
    # 03OCT2021
    def cvcc(self, cp, vin):

        # turn on unit first
        self.ac_turn_on(vin)

        # select digital control
        self.click('SELECT_TEST_OPTION')
        sleep(0.5)
        self.click('SELECT_TEST_DIGITAL_CONTROL')
        ...
        self.change_value('I2C_SEND_WRITE_BIT', 254)
        ...
        self.click('SELECT_TEST_OPTION')
        sleep(0.5)
        self.click('SELECT_LINE_LOAD_REGULATION')
        sleep(0.5)
        self.click('LOAD_ENABLE_BUTTON')
        sleep(0.5)
        self.click('SELECT_REGULATION_TEST_EXIT_BUTTON')
    ...
    def PWM_DIMMING(self, start, end, step, frequency, soak, delay_per_line, delay_per_load):
        ...
        self.click('ACTIVATE_LOAD_SETTINGS_FOR_ATE')
        self.click('ACTIVATE_LOAD_SETTINGS_FOR_ATE_OK_BUTTON')
        self.change_value('ACTIVATE_LOAD_SETTINGS_FOR_ATE_TEST_TYPE', "PWM")
        self.click('MULTIPROTOCOL_CONTROL_EXIT_BUTTON')
        self.click('LOAD_SETTINGS_EXIT_BUTTON') 
    ...
    def ac_turn_on(self, vin):
        ...
        if vin >= 180 and vin <= 265: freq = 50
        else: freq = 60
