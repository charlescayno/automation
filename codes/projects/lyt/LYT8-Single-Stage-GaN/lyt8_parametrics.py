...
def operation():
    ...
    for iout in iout_list:
        ...
        for vin in vin_list:
            ...
            df = collect_data(df, iout, vin)

            print(df[(df.led_load == iout)])   
            create_output_excel_result(dst, df_header_list, df, iout, vin, test)


        print(f"\n\nFinal Data at {iout}A: ")
        print(df)
