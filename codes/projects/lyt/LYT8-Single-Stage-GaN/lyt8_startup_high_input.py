...
def operation():
    ...
    if load_type == 'cc':
        for iout in iout_list:
            for end in range(230,291,1):
                line_ovp_startup_at_high_input(df, end_voltage=end, load=iout)


    if load_type == 'cr':
        for cr in cr_list:
            for end in range(230,291,1):
                line_ovp_startup_at_high_input(df, end_voltage=end, load=cr)
