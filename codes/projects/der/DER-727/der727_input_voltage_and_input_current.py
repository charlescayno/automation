...
def scope_settings():
    ...
    scope.channel_settings(state='ON', channel=3, scale=0.5, position=1, label="Input Current",
                            color='GREEN', rel_x_position=40, bandwidth=20, coupling='DCLimit', offset=0)
    
    scope.channel_settings(state='ON', channel=4, scale=100, position=-2, label="Input Voltage",
                            color='PINK', rel_x_position=60, bandwidth=20, coupling='AC', offset=0)
...
def operation():
    ...
    for led in led_list:
        ...
        for vin in vin_list:
            ...
            if vin >= 230:
                scope.channel_scale(3, 0.3)
                scope.channel_scale(4, 200)
            else:
                scope_settings()

            # find_trigger(channel=trig_channel, trigger_delta=1)
            
            scope.run_single()
            sleep(3)
            
            discharge_output()
            filename = f"Vin Iin Norm Op - {led}V, {vin}Vac.png"
