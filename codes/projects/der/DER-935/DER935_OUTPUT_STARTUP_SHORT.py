...
test = "OUTPUT STARTUP SHORT (TSP15H150S S1G)"
...
def operation():
    ...
    for led in led_list:
        ...
        for vin in vin_list:
            ...
            filename = f'{led}V, {vin}Vac, {ac.frequency}Hz, Startup Short.png'
            ...
            soak(9*timeperdiv+timeperdiv) 
            discharge_output()
            filename = f'{led}V, {vin}Vac, {ac.frequency}Hz, After Startup Short.png'
            scope.get_screenshot(filename, waveforms_folder)
            waveform_counter+=1
            print(filename)
