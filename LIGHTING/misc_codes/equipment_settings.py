...
from powi.equipment import ACSource, PowerMeter, ElectronicLoad, Oscilloscope, LecroyScope, LEDControl, Keithley_DC_2230G, RnS_DC_HMPSeries, Tektronix_SigGen_AFG31000, Keysight_33500B, DCSource
...
def _connect(label, factory):
    """Attempt to connect to an instrument. Returns the instance or None on failure."""
    try:
        instance = factory()
        print(f"  [OK]  {label}")
        return instance
...
pml     = _connect("Power Meter Load 1", lambda: PowerMeter(EQUIPMENT_ADDRESS.POWER_METER_LOAD_1))
pml2    = _connect("Power Meter Load 2", lambda: PowerMeter(EQUIPMENT_ADDRESS.POWER_METER_LOAD_2))
pml3    = _connect("Power Meter Load 3", lambda: PowerMeter(EQUIPMENT_ADDRESS.POWER_METER_LOAD_3))
eload   = _connect("Electronic Load",    lambda: ElectronicLoad(EQUIPMENT_ADDRESS.ELOAD))
sig_gen = _connect("Signal Generator",   lambda: Tektronix_SigGen_AFG31000(EQUIPMENT_ADDRESS.SIG_GEN))
dc      = _connect("DC Source",          lambda: DCSource(EQUIPMENT_ADDRESS.DC_SOURCE))
dimmer  = _connect("Dimmer",             lambda: RnS_DC_HMPSeries(EQUIPMENT_ADDRESS.DC_SOURCE_DIMMER))
scope   = _connect("Oscilloscope",       lambda: Oscilloscope(EQUIPMENT_ADDRESS.SCOPE))
led_ctrl = _connect("LED Control",       lambda: LEDControl())
print("Done.\n")

# try:
#     from i2c_reader import read_eeprom as _read_eeprom
# except ImportError:
#     _read_eeprom = None
...
class EQUIPMENT_FUNCTIONS():
    ...
    class SCOPE():
        ...
        def SCOPE_SCREENSHOT_LOOPER(self, filename, path):
            ...
            while '' == input(">> Press ENTER to continue capture. To stop press other keys to stop: "):
                fname = filename.split('.png')[0] + f'_{count}' + '.png'
                count += 1
    ...
    def DC_SOURCE_TURN_ON(self, voltage):
        if voltage > 10:
            voltage = 10
        dc.voltage = voltage
        dc.turn_on()

    def DC_SOURCE_TURN_OFF(self):
        dc.turn_off()
    ...
    def DISCHARGE_OUTPUT(self, times):
        ...
        for _ in range(times):
            ...
            sleep(1)
            for ch in range(1, 9):
                eload.channel[ch].turn_off()
                eload.channel[ch].short_off()
            sleep(1)
    ...
    def COLLECT_DATA_SINGLE_OUTPUT_CC_LOAD_1_PDEL(self, vin, vout, cc, percent):
        ...
        try:
            pkpk = self._sigfig(scope_measurement['Peak to peak'], 4)
            pkpk = pkpk*1000
        except:
            pkpk = 0
    ...
    def COLLECT_DATA_SINGLE_OUTPUT_RBIAS(self, vin, iteration, rbias):
        ...
        try:
            vbias = self._sigfig(vbias_measurement['RMS'], 4)
        except:
            vbias = 0

        vbp_measurement = scope.get_measure_dict(2)
        try:
            vbp = self._sigfig(vbp_measurement['RMS'], 4)
        except:
            vbp = 0
    ...
    def COLLECT_DATA_SINGLE_OUTPUT_CC_LOAD_NORMAL(self, vin, vout, cc):
        ...
        output_list = [vin, ac.set_freq(vin), vac, iin, pin, pf, thd, vo1, io1, po1, vreg1, eff]

        return output_list
    
    def COLLECT_DATA_SINGLE_OUTPUT_CC_LOAD_NORMAL_with_ripple(self, vin, vout, cc, channel_list):

        vac, iin, pin, pf, thd, vo1, io1, po1 = self._pm_measurements()
        vo3, io3, po3 = self._pm_measurements3()

        try: vreg3 = self._sigfig(100*(vo3-vout)/vo3, 6)
        except: vreg3 = "NaN"
        try: ireg3 = self._sigfig(100*(io3-(cc*1000))/(cc*1000), 6)
        except: ireg3 = "NaN"
        try: eff = self._sigfig(100*(po3)/pin, 6)
        except: eff = "NaN"

        output_list = [vin, ac.set_freq(vin), vac, iin, pin, pf, thd, vo3, io3, po3, vreg3, eff]
    ...
    #     isense_adc, vbus_adc, chs, ce, trough, peak, vcoil_adc, duty_holder, rp8, pin_adc, pf_adc, pout_adc, eff_adc, vin_adc, state, status_if_pout_comms_received, fod_status = _read_eeprom()
    ...
    #     comms = self._sigfig(scope_comms_measurement['Max'], 4)/1000
    #     fsw = self._sigfig(scope_measurement['Frequency'], 4)/1000
    #     duty = self._sigfig(scope_measurement['Pos. duty cycle'], 2)
    #     # vcoil_max = self._sigfig(scope_measurement2['Max'], 4)
    #     scope.run()
