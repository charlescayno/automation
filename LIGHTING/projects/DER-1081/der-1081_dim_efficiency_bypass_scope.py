...
def scope_restore_setup():
    if not bypass_oscilloscope:
        scope.write(
            "MMEM:RCL "
            "'C:/Users/Public/Documents/Rohde-Schwarz/RTx/SaveSets/2025/DER-1081/DEEP DIMMING.dfl'"
        )


def main():

    # Restore oscilloscope setup only if scope is enabled
    scope_restore_setup()

    header_list = GENERAL_CONSTANTS.HEADER_LIST_1CV_1CC_PARAMETRICS_with_dimming[:]

    # Append scope labels only when scope is used
    if not bypass_oscilloscope:
        for channel in scope_channel_list:
            header_list = EQUIPMENT_FUNCTIONS().APPEND_SCOPE_LABELS(
                header_list,
                channel,
            )
    ...
    EQUIPMENT_FUNCTIONS().MULTIPLE_ELOAD_CC_ON(
        iout_nom_1,
        iout_nom_2,
        iout_nom_3,
    )
        ...
        for vin in vin_list:
            ...
            for load_1 in range(100, -1, -1):
                ...
                filename = (
                    f"{test_name} "
                    f"{vin}VAC "
                    f"{vout_nom_1}V{iout_nom_1}A "
                    f"{vout_nom_2}V{iout2}A"
                )
                ...
                output_list = (
                    EQUIPMENT_FUNCTIONS()
                    .COLLECT_DATA_1CV_1CC_PARAMETRICS_with_dimming(
                        vin,
                        vout_nom_1,
                        vout_nom_2,
                        iout_nom_1,
                        ...
                        dim_freq,
                        load_1,
                        [] if bypass_oscilloscope else scope_channel_list,
                    )
                )
                ...
                export_to_excel(
                    ...
                    waveforms_folder,
                    output_list,
                    excel_name=f"{vout_nom_1}V {test_name}",
                    sheet_name=f"{vout_nom_1}V {test_name}",
                    anchor="A1",
                )
