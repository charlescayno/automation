...
test_name = f"IIN VDS FSWSteady State"
...
def main():
    ...
    # EQUIPMENT_FUNCTIONS().SCOPE().POSITION_SCALE(time_position=20, time_scale=5E-6)
    EQUIPMENT_FUNCTIONS().SCOPE().CHANNEL_SETTINGS(state='ON', channel=1, scale=0.4, position=0, label="IIN", color='LIGHT_BLUE', rel_x_position=30, bandwidth=500, coupling='DCLimit', offset=0)
    EQUIPMENT_FUNCTIONS().SCOPE().CHANNEL_SETTINGS(state='ON', channel=2, scale=100, position=-4, label="VDS", color='YELLOW', rel_x_position=40, bandwidth=500, coupling='DCLimit', offset=0)
