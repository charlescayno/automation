...
def main():
    ...
    for vin in vin_list:
        ...
        EQUIPMENT_FUNCTIONS().ANALOG_DC_ON(1, 0, 0.5)
        input("Start dimming")

        EQUIPMENT_FUNCTIONS().AC_TURN_ON(vin)
        soak(soak_time_per_line)

        for load_1 in np.arange(0, 1.01, 0.01):

            EQUIPMENT_FUNCTIONS().ANALOG_DC_ON(1, load_1, 0.5)
        ...
        for load_1 in np.arange(1, 10.1, 0.1):
