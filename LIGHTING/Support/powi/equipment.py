...
class ACSource(Equipment):
    ...
    def current_peak(self, peak):
        self.write(f":CURRent:PEAK {peak}")
...
class LecroyScope(Equipment):
    def __init__(self, address, comm_type='lan_lecroy', timeout=1E6):
        super().__init__(address, comm_type, timeout)
        print("lecroy")
...
class RnS_DC_HMPSeries(Equipment):
    def __init__(self, address, comm_type='gpib', timeout=20000):
        super().__init__(address, comm_type, timeout)
 
 #################### SET VOL&CURR #################
    def set_volt_curr(self, channel, voltage = 0.0, current = 0.0):
        self.write(f'INST OUT{channel}')
        self.write(f'APPL {voltage}, {current}')
 
    def channel_state(self, channel, state): #Individual
        self.write(f'INST OUT{channel}')
        self.write(f'OUTP {state}')
 
    def channel_state_all(self, state): #For all channels
        self.write(f'OUTP:GEN {state}')          
...
class Keysight_33500B(Equipment):
    def __init__(self, address, comm_type='gpib', timeout=20000):
        super().__init__(address, comm_type, timeout)
...
class LEDControl():
    ...
    def voltage(self, voltage: int):
        ...
        elif voltage > 36 and voltage <= 40:
            self.RELAY1.write(0)
            self.RELAY2.write(1)
            self.RELAY3.write(0)
            print(f"LED set to {voltage}V.")
        elif voltage <= 36 and voltage > 0:
            self.RELAY1.write(0)
            self.RELAY2.write(0)
            self.RELAY3.write(1)
            print(f"LED set to {voltage}V.")
