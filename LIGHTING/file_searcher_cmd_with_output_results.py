...
def search_files():
    ...
    for root, dirs, files in os.walk(folder_path):
        for dir in dirs:
            if keyword in dir.lower():
                ...
                ws.cell(row=row, column=2).hyperlink = Hyperlink(dir_path)
                row += 1
        for file in files:
            if keyword in file.lower():
                ...
                ws.cell(row=row, column=2).hyperlink = Hyperlink(file_path)
                row += 1
                files_found = True
    ...
    if not files_found:
        ...
        print(f"Search results saved to {os.getcwd()}/search_results.xlsx")
